...
impl Shell {
    pub fn new() -> Shell {
        ...
        Self {
            ...
            #[cfg(target_os = "macos")]
            hostname: gethostname().into_string().unwrap().replace(".local", ""),
            #[cfg(target_os = "linux")]
    ...
    fn launch_command(&mut self, command: &str, args: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    fn operate_command(&mut self, cmd: &str, args: &mut [String]) {
        match cmd {
            "cd" => {
                let path_idx = args
                    .iter()
                ...
                match self.launch_command(cmd, args) {
                    Ok(_) => {
            ...
            _ => {
                if let Err(error) = self.launch_command(cmd, args) {
                    println!("error: {}", error);
                }
            }
        }
    ...
    pub fn listen_to_stdin(&mut self) {
        loop {
            ...
            let (cmd, mut args) = match self.parse_input(&self.input) {
            ...
            self.operate_command(&cmd, &mut args);
            self.input.clear();
        }
    }
}
